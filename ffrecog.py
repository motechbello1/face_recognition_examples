...
def main():
    ...
    # Face recognition algorith using known face encodings on each frame of the unknown video stream
...
def greet(name, known_dir, place=True):
    ...
    f.write(name+" ,"+str(ct)+'\n')
    f.close()
    sound = f"Hello {name}. Welcome to ENCAR" if place else "Hello" + name
    engine.say(sound)
    engine.runAndWait()
