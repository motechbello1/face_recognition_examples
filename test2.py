...
def main():
    ...
    if len(sys.argv) != 2:
        sys.exit("Usage: python loop.py name")
...
for file in os.listdir(os.path.join(os.getcwd(), './img/known/')):
    if file.endswith(".jpg") or file.endswith(".png") or file.endswith(".JPG"):
        person_name = file[:-4]
        print(f"Training on {person_name}")
        image_path = f"{os.path.join(os.getcwd(), './img/known/')}{file}"
...
# Display wait image and wait for key press
wait_image = cv2.imread(os.path.join(os.getcwd(), "img/wait_image.jpg"))
